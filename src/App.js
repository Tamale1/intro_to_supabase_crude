...
function App() {
  ...
  const fetchTodos = async ()=>{
    //select * from todos order by id asc(sql)
  ...
  const addTodo = async (id, task, is_completed)=>{
    //insert into todos (tasks,is_completed) values(newTask,false)
  ...
  const updateTodo = async (id)=>{
    //update todos set task = editingText where id = id(sql)
  ...
  const toggleCompletion = async (id, is_completed)=>{
    //update todos set is_completed =  !is_completed where id = id (sql)
    const {error}= await supabase
    .from('todos')
  ...
  return (
    <div className='App' style={{display: 'flex', flexDirection: 'column',alignItems: 'center'}}>
      ...
      <ul style={{listStyleType: 'none', padding: 0}}>
        {todos.map((todo)=>(<li key={todo.id} style={{marginBottom: '1rem', textAlign: 'center'}}>
          {editingId === todo.id ?(<input type='text' value={editingText} onChange={(e)=>setEditingText(e.target.value)} onBlur={()=>updateTodo(todo.id)}></input>
          ):
          (
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <div>
